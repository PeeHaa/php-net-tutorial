...
Another useful tool for debugging your PHP code is [Xdebug][x-debug]. Xdebug formats (amongst others) the error and the `var_dump` outputs of PHP. Xdebug is not installed by default on most PHP installation, but can be downloaded from [their site][xdebug].
